...
## 2025-05-19 - Reuse Regex Results and Use Exact String `.count()`
**Learning:** Performing multiple regex searches on the same large string for similar patterns (e.g., `\b(\d+)\s+\d+\s+obj\b` and `\b\d+\s+\d+\s+obj\b`) is highly redundant. Furthermore, using regex word boundary matching (`\bendobj\b`) is up to 50x slower than python's native exact string counting `txt.count("endobj")` for standalone, unique keywords within PDFs.
**Action:** When gathering matches via regex to populate sets or lists, save the list of matches. You can subsequently query its `len()` rather than running another regex to just get the count. For counting specific, known unique literal keywords (like `endobj` in PDF structures) use `string.count("word")` over `len(re.findall(r"\bword\b", string))`.
