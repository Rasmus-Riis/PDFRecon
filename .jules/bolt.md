...
## 2025-05-21 - Optimize tree_item UI calls in exporter.py
**Learning:** Calling `tree_item(item_id, "values")` and `tree_item(item_id, "tags")` inside a nested dictionary mapping or search lookup loops for thousands of tree items causes severe lag due to Python-to-Tkinter C-level bridge overhead.
**Action:** When extracting data from a UI tree (like Tkinter's `Treeview`), map all relevant items directly to Python dictionaries ahead of time to avoid repetitive UI calls inside processing loops.
