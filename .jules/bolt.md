...
## 2025-05-22 - Avoid datetime.strptime for fixed-format dates
**Learning:** Using `datetime.strptime` inside performance-critical parsing loops introduces significant overhead due to format string parsing and locale locks. For fixed-length date strings like 14-digit PDF dates (`YYYYMMDDHHMMSS`), directly instantiating the `datetime` object via integer slicing is roughly ~3-4x faster. For ISO formatted strings, `datetime.fromisoformat()` is orders of magnitude faster than `strptime`.
**Action:** Always prefer `datetime.fromisoformat()` for ISO strings or direct `datetime(int(y), ...)` instantiation via slicing for fixed-length date strings instead of `strptime` in high-frequency parsing paths.
