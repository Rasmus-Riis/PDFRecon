...
## 2025-05-20 - Fast datetime parsing replacing strptime
**Learning:** `datetime.strptime` is noticeably slow due to string format parsing and acquiring locale locks. For large volumes of standard or predictable datetime strings (e.g., ISO formats or 14-digit PDF datestamps like `YYYYMMDDHHMMSS`), direct instantiation or `fromisoformat` can be significantly faster.
**Action:** When parsing datetimes in performance-critical sections with a fixed, known string length, replace `datetime.strptime(date_str, "%Y%m%d%H%M%S")` with direct initialization: `datetime(int(s[0:4]), int(s[4:6]), int(s[6:8]), int(s[8:10]), int(s[10:12]), int(s[12:14]))`. Use `datetime.fromisoformat(date_str)` for ISO 8601 strings, which provides a ~25x speedup.
