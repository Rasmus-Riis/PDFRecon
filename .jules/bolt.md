...
## 2025-05-25 - Avoid datetime.strptime format parsing overhead
**Learning:** `datetime.strptime` is notoriously slow because it relies on internal pure-Python regex parsing and locale locks. For standard or fixed formats, alternative parsing is vastly superior. `datetime.fromisoformat()` is an optimized C-level parser that can provide a 10x-20x speedup for ISO 8601 strings. For custom fixed-length strings (like PDF timestamps `YYYYMMDDHHMMSS`), direct integer slicing `datetime(int(s[0:4]), ...)` can provide a 5x speedup.
**Action:** In performance-critical loops (e.g. metadata extraction or timeline generation), always prefer `datetime.fromisoformat()` for standard ISO dates, or string slicing with `datetime(...)` instantiation for simple fixed-length custom formats instead of `datetime.strptime`.
