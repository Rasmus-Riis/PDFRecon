...
## 2025-05-20 - Fast XREF Duplicate Object ID Checking
**Learning:** When scanning multiple XREF tables to detect duplicate object IDs (shadow attacks), accumulating IDs in a list and using an `in` check inside the loop creates an O(N^2) bottleneck. For large PDFs with many objects and multiple revisions, this can cause significant parsing delays.
**Action:** When accumulating items and checking for duplicates inside a loop in Python, always use a `set` for O(1) membership testing rather than an `in` check against a `list` (e.g., `all_objects = set()` instead of `all_objects = []`).
