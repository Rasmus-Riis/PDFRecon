...
## 2025-05-20 - Cache and reuse regex matches to prevent redundant scanning
**Learning:** Running identical or near-identical regular expressions multiple times over the same large text buffers (e.g. searching for object definitions in a PDF string to first get their IDs, and then again later to get the total count) introduces unnecessary overhead.
**Action:** Always inspect functions for repeated `re.findall` or `re.search` calls using the same patterns. Execute the regex once, cache the output list, and reuse it for length checks (`len()`), comprehensions (`{int(m) for m in matches}`), or other downstream operations to avoid redundant linear scans.
