...
## 2025-05-20 - Faster datetime parsing using integer slicing and fromisoformat
**Learning:** `datetime.strptime` is slow due to parsing format strings and locale lock overhead. When dealing with fixed format strings like `YYYYMMDDHHMMSS` (typical in PDF dates), passing sliced integers directly to `datetime()` constructor (`datetime(int(s[0:4]), ...)`) yields up to ~4x speedups. For ISO 8601 formatted strings, `datetime.fromisoformat()` is exponentially faster (up to ~35x) than `strptime`.
**Action:** Always prefer integer slicing and `datetime()` constructor for fixed-width date strings, and `datetime.fromisoformat()` for ISO-8601 strings in performance-critical loops instead of `datetime.strptime()`.
