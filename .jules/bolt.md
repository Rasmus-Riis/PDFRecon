...
## 2025-05-19 - Safe OS-Conditional Imports
**Learning:** Avoid using OS-conditional local imports (e.g., `import subprocess` inside an `if sys.platform == "win32":` block) if the imported module might be referenced in shared exception handling or fallback code paths outside the conditional block. During testing, this caused an `UnboundLocalError` when `subprocess.TimeoutExpired` was caught later in the function on non-Windows platforms.
**Action:** When making minor modifications, preserve existing global imports or ensure that module-level dependencies are accessible to all code paths that might need them, particularly in cross-platform error handling blocks.
