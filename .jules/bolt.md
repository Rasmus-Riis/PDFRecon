...
## 2024-05-24 - Optimize datetime parsing
**Learning:** Using `datetime.strptime` inside high-frequency loops introduces significant overhead due to format string parsing and locale locks.
**Action:** For fixed-length strings like 14-digit PDF dates (`YYYYMMDDHHMMSS`), instantiate `datetime` directly using integer slicing. For ISO formatted strings, use `datetime.fromisoformat()`, which is significantly faster.
