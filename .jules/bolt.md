...
## 2024-05-19 - Replace strptime with slicing and fromisoformat
**Learning:** `datetime.strptime` is a known bottleneck due to locale lock acquisition and C-level regex format string parsing. This applies particularly when reading multi-gigabyte or log-heavy outputs containing static timestamp structures.
**Action:** When working in performance-critical areas dealing with fixed 14-digit (`YYYYMMDDHHMMSS`) or ISO timestamps, leverage integer string slicing paired with `datetime()` instantiation or `datetime.fromisoformat()` instead of `datetime.strptime`. Always verify parsing behaves identically for edge cases.
