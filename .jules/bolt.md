...
## 2025-06-27 - Fast Date Parsing with Direct Instantiation
**Learning:** Using `datetime.strptime` inside loops to parse fixed-format strings (like 14-digit PDF dates or ISO timestamps) introduces significant overhead because it involves format string evaluation and locale locking.
**Action:** When parsing known, fixed-length date strings in performance-sensitive contexts, bypass `datetime.strptime`. Instead, use direct string slicing paired with `datetime(...)` instantiation for simple formats (e.g., `YYYYMMDDHHMMSS`), or use `datetime.fromisoformat(...)` for ISO 8601 strings. This avoids the heavy regex and C-level overhead of `strptime`, yielding much faster execution times on large documents.
