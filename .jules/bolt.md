...
## 2024-05-27 - [Optimize `export_to_html` UI Lookups]
**Learning:** Found a severe O(N^2) performance bottleneck in HTML report generation where iterating over `report_data` performed a linear search through the GUI tree items for every row using `tree_get_children` and `tree_item`.
**Action:** When performing data exports that need to correlate with UI elements, always pre-compute a mapping dictionary (e.g. by file path) before entering the export loop to achieve O(1) lookups and prevent execution bottlenecks on large datasets.
