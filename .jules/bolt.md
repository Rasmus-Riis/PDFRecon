...
## 2025-05-20 - Fast Date Parsing with fromisoformat
**Learning:** In hot loops, Python's `datetime.strptime` with format strings incurs significant regex parsing and locale locking overhead. Using `datetime.fromisoformat` for ISO strings or directly instantiating `datetime(y, m, d, ...)` for fixed-length dates (like `YYYYMMDDHHMMSS` in PDFs) is roughly 4x-30x faster.
**Action:** Always prefer `datetime.fromisoformat` or direct string slicing + instantiation over `datetime.strptime` when parsing standardized timestamps within loops.
