...
## 2024-05-02 - Export Loop Instantiation Overhead
**Learning:** Creating openpyxl style objects (like `Alignment`) inside tight loops significantly slows down Excel generation. The `openpyxl` library doesn't automatically deduplicate these style objects, causing unnecessary memory allocation and overhead for every cell.
**Action:** Always pre-instantiate `openpyxl` style objects (`Alignment`, `Font`, `PatternFill`, etc.) outside of loops and re-use the shared instance for all applicable cells. Also, cache dictionary `.get` methods for O(1) lookups inside these hot loops.
