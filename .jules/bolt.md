...
## 2025-05-18 - Pre-compute path-to-tag mapping for HTML export
**Learning:** During HTML export, matching each row to its corresponding UI tag class by repeatedly iterating over all UI tree children resulted in O(N^2) time complexity. This is extremely inefficient when dealing with large reports.
**Action:** Pre-compute a dictionary mapping the file path to its tag class by iterating over the UI tree children exactly once before starting the export loop. This changes the O(N^2) search operation into an O(N) pre-computation followed by O(1) lookups per row.
