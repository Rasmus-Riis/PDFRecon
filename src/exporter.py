...
def export_to_html(file_path, report_data: list, file_annotations: dict, all_scan_data: dict,
    ...
    try:
        ...
        # ⚡ Bolt Optimization: Pre-map Tkinter Treeview items to a native Python dictionary
        # This achieves O(1) lookups and prevents O(N^2) performance bottlenecks, avoiding severe lag
        # caused by the Python-to-Tkinter C-level bridge overhead.
        path_to_tag_class = {}
        ...
        for i, values in enumerate(report_data):
            path_str = values[4] if len(values) > 4 else ""
