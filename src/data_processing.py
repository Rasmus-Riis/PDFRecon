...
class DataProcessingMixin:
    ...
    def _extract_touchup_text(self, doc, capture_runs=False):
        ...
        Works by masking everything *outside* a TouchUp marked-content region
        and letting PyMuPDF extract what remains, which handles layout and
        line breaks. That path decodes through the font's ToUnicode CMap, so
        it produces garbled output when the CMap is missing or incomplete.

        With ``capture_runs=True`` the raw string operands and their font
        resource are also collected during the same walk, so
        :mod:`src.cid_decoder` can decode them by other means. Returns
        ``(page_results, runs, pdf_bytes)`` in that case and just
        ``page_results`` otherwise, leaving existing callers unchanged.
        """
        import pikepdf
        import io
        import logging
                        ...
                        for operands, operator in ops:
                            ...
                            if op_name in {"BDC", "BMC"}:
                                ...
                                if "TouchUp" in tag:
                                    ...
                                    # "tag properties DP" - same operand order
                                    # as BDC.
                                    prop_key = operands[1] if len(operands) > 1 else None
                                    if properties and prop_key is not None:
                                        try:
                            ...
                            if not is_inside_touchup and op_name in {"Tj", "TJ", "'", '"'}:
                                ...
                                # two-byte Identity-H font - exactly the kind
                                # this decoder exists for - the pair 0x20 0x20
                                # is character code 0x2020, which renders as a
                                # dagger, so masked text came back as strings
                                # of daggers instead of nothing.
                                #
                                ...
                                if capture_runs and is_inside_touchup and op_name in {"Tj", "TJ", "'", '"'}:
                                    self._capture_touchup_run(
                                        captured_runs, pikepdf, page_num,
                                        current_font, op_name, operands)
                                new_ops.append((operands, operator))

                        # pikepdf.Page has no set_contents(); assigning /Contents
                        # through the page object is the supported way to
                        # replace a content stream. Calling a method that does
                        # not exist raised AttributeError into the handler
                        # below, which meant masking silently never happened
                        # and this function returned the whole page's text.
    ...
    @staticmethod
    def _capture_touchup_run(captured_runs, pikepdf, page_num, font_resource,
                             op_name, operands):
        """
    ...
    def _decode_touchup_runs(self, captured_runs, pdf_bytes):
        ...
        carrying the text, method, confidence, evidence and alternatives for
        each run, and *custody* summarises which tiers ran and the hashes of
        the tables and fonts involved.

        Runs whose font cannot be located are still reported, with the reason,
        rather than dropped: an examiner needs to know a run existed even when
        it could not be decoded. Any failure here leaves the existing TouchUp
        ...
        try:
            ...
            # Map each page's font resource names to their xrefs. The xrefs
            # come from the same byte stream the runs were captured from, and
            # PyMuPDF's tobytes() preserves the original numbering, so they
            # match what an examiner sees opening the file itself.
            fonts_by_page = {}
