    ...
    try:
        def parse_pdf_date(date_str):
            ...
            if len(clean) >= 14:
                try:
                    c = str(clean)[:14]
                    # ⚡ Bolt Optimization: Replace datetime.strptime with direct instantiation for speed
                    return datetime(int(c[0:4]), int(c[4:6]), int(c[6:8]), int(c[8:10]), int(c[10:12]), int(c[12:14]))
                except Exception:
                    pass
            return None
        ...
        if len(items) > 1:
            ...
            for i in range(len(items) - 1):
                try:
                    ...
                    # Convert to datetime
                    # ⚡ Bolt Optimization: Use datetime.fromisoformat instead of strptime for standard ISO formats
