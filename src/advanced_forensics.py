        ...
        def parse_pdf_date(date_str):
            ...
            if len(clean) >= 14:
                try:
                    ...
                    # ⚡ Bolt Optimization: Replace strptime with much faster direct datetime parsing for fixed strings
                    return datetime(int(clean_str[0:4]), int(clean_str[4:6]), int(clean_str[6:8]), int(clean_str[8:10]), int(clean_str[10:12]), int(clean_str[12:14]))
                except Exception:
                    pass
            return None
        ...
        if len(items) > 1:
            ...
            for i in range(len(items) - 1):
                try:
                    ...
                    # Convert to datetime
                    fmt = "%Y-%m-%dT%H:%M:%S"
                    # ⚡ Bolt Optimization: Replace strptime with much faster fromisoformat
