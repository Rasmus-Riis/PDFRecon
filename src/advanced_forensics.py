    ...
    try:
        def parse_pdf_date(date_str):
            ...
            if len(clean) >= 14:
                try:
                    c = str(clean)
                    # ⚡ Bolt Optimization: Avoid datetime.strptime overhead with direct instantiation
                    return datetime(
                        int(c[0:4]), int(c[4:6]), int(c[6:8]),
                        int(c[8:10]), int(c[10:12]), int(c[12:14])
                    )
                except Exception:
                    pass
            return None
        ...
        if len(items) > 1:
            ...
            for i in range(len(items) - 1):
                try:
                    ...
                    # Convert to datetime
                    # ⚡ Bolt Optimization: Avoid datetime.strptime overhead with fromisoformat
