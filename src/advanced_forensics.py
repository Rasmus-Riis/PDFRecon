...
def detect_timestamp_mismatches(txt: str, doc, indicators: dict):
    ...
    try:
        def parse_pdf_date(date_str):
            ...
            if len(clean) >= 14:
                try:
                    # ⚡ Bolt Optimization: Replace datetime.strptime with faster slicing for YYYYMMDDHHMMSS format
                    s = str(clean)[:14]
                    return datetime(
        ...
        if len(items) > 1:
            ...
            for i in range(len(items) - 1):
                try:
                    ...
                    # Convert to datetime
                    # ⚡ Bolt Optimization: Replace datetime.strptime with faster datetime.fromisoformat
