...
def detect_timestamp_mismatches(txt: str, doc, indicators: dict):
    ...
    try:
        def parse_pdf_date(date_str):
            ...
            if len(clean) >= 14:
                try:
                    # ⚡ Bolt Optimization: Use direct datetime instantiation for fast date string parsing
                    d_str = str(clean)[:14]
                    return datetime(int(d_str[0:4]), int(d_str[4:6]), int(d_str[6:8]), int(d_str[8:10]), int(d_str[10:12]), int(d_str[12:14]))
                except Exception:
                    pass
            return None
        ...
        if len(items) > 1:
            ...
            for i in range(len(items) - 1):
                try:
                    ...
                    # Convert to datetime
                    # ⚡ Bolt Optimization: Use datetime.fromisoformat for fast ISO date parsing
