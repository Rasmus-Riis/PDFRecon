...
def detect_timestamp_mismatches(txt: str, doc, indicators: dict):
    ...
    try:
        def parse_pdf_date(date_str):
            ...
            if len(clean) >= 14:
                try:
                    # ⚡ Bolt Optimization: Replace slow strptime with fast slicing for 14-digit fixed PDF dates
                    d = str(clean)[:14]
                    return datetime(int(d[0:4]), int(d[4:6]), int(d[6:8]), int(d[8:10]), int(d[10:12]), int(d[12:14]))
                except Exception:
                    pass
            return None
        ...
        if len(items) > 1:
            ...
            for i in range(len(items) - 1):
                try:
                    ...
                    # Convert to datetime
                    # ⚡ Bolt Optimization: Replace slow strptime with much faster fromisoformat
