    ...
    try:
        def parse_pdf_date(date_str):
            ...
            if len(clean) >= 14:
                try:
                    s = str(clean)[:14]
                    # ⚡ Bolt Optimization: Replace datetime.strptime with direct instantiation using integer slicing for fixed-length dates
        ...
        if len(items) > 1:
            ...
            for i in range(len(items) - 1):
                try:
                    ...
                    # ⚡ Bolt Optimization: Replace datetime.strptime with native datetime.fromisoformat to avoid format string parsing overhead
