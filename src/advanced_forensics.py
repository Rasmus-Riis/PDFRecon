...
def detect_emails_and_urls(txt: str, indicators: dict):
    ...
    try:
        ...
        for email in raw_emails:
            ...
            # 1. Too many numbers
            if sum(1 for c in email if c.isdigit()) > len(email) * 0.4:
                continue

            # 2. Vowel-to-consonant ratio
            vowels = sum(1 for c in email if c.lower() in 'aeiouy')
            consonants = sum(1 for c in email if c.lower()
                             in 'bcdfghjklmnpqrstvwxz')
            if vowels == 0:
                continue
            if (consonants / len(email)) > 0.8:
                continue
            ...
            if uppers >= 2 and lowers >= 2:
                continue  # Real domains are rarely camelCase in extracted text
        ...
        urls = set()
        exclude_domains = {'ns.adobe.com', 'www.w3.org', 'purl.org',
                           'xml.org', 'schemas.microsoft.com', 'iptc.org'}
        for url in raw_urls:
    ...
    try:
        ...
        for path in raw_paths:
            ...
            if len(parts) >= 2 and len(parts[0]) >= 3 and len(parts[1]) >= 3:
                ...
                # Too many numbers?
                if sum(1 for c in path if c.isdigit()) > len(path) * 0.4:
                    continue
    ...
    try:
        ...
        if doc and hasattr(doc, 'is_encrypted') and doc.is_encrypted:
            ...
            if hasattr(doc, 'needs_pass') and doc.needs_pass:
                indicators['PasswordRequired'] = {
                    'status': 'User password required'}
            else:
                indicators['EncryptedButOpen'] = {
                    'status': 'Opened without password (empty or known)'}
        ...
        if "/Encrypt" in txt and re.search(r'/Encrypt\s+\d+\s+\d+\s+R', txt):
            if 'Encrypted' not in indicators:
                indicators['EncryptionDictionary'] = {
                    'status': 'Encryption dictionary present'}
...
def detect_hidden_text_patterns(txt: str, doc, indicators: dict):
    ...
    try:
        ...
        if "Tr" in txt and re.search(r'\s3\s+Tr\b', txt):
            indicators['InvisibleTextMode'] = {
                'status': 'Text rendering mode 3 (invisible) detected'}

        # Check for white color (1 1 1 RG or #FFFFFF)
        white_color_matches = 0
        if "rg" in txt or "RG" in txt:
            white_color_matches = len(re.findall(
                r'\b1\s+1\s+1\s+(rg|RG)\b', txt))
...
def detect_attachments(doc, txt: str, indicators: dict):
    ...
    try:
        ...
        if "embeddedfile" in txt_lower and re.search(r'/Type\s*/EmbeddedFile', txt, re.IGNORECASE):
            ...
            embedded_count = len(re.findall(
                r'/Type\s*/EmbeddedFile', txt, re.IGNORECASE))
...
def detect_3d_and_multimedia(txt: str, indicators: dict):
    ...
    try:
        ...
        if "/Sound" in txt and re.search(r'/Subtype\s*/Sound', txt):
            indicators['SoundAnnotations'] = {
                'status': 'PDF contains sound annotations'}

        if ("/Movie" in txt or "/Screen" in txt) and (re.search(r'/Subtype\s*/Movie', txt) or re.search(r'/Subtype\s*/Screen', txt)):
            indicators['VideoContent'] = {
                'status': 'PDF contains video/multimedia content'}
    ...
    try:
        ...
        if "pdfaid:part" in txt_lower and re.search(r'pdfaid:part', txt, re.IGNORECASE):
            part_match = re.search(
                r'pdfaid:part>(\d+)</pdfaid:part', txt, re.IGNORECASE)
            conformance_match = re.search(
                r'pdfaid:conformance>([A-Z])</pdfaid:conformance', txt, re.IGNORECASE)

            part = part_match.group(1) if part_match else 'Unknown'
            conformance = conformance_match.group(
                1) if conformance_match else 'Unknown'
    ...
    try:
        ...
        if header_offset > 0 and header_offset <= 1024:
            ...
            if header_offset >= 512 or detected_format:
                ...
                logging.warning(
                    f"Polyglot file detected: PDF header at offset {header_offset}")
...
def run_advanced_forensics(txt: str, doc, filepath: Path, indicators: dict):
    ...
    try:
        ...
        pdf_bytes = filepath.read_bytes() if filepath and filepath.exists(
        ) else txt.encode('latin-1', errors='ignore')
        ...
        logging.warning(
            f"Error in advanced forensics for {filepath.name}: {e}")
...
def detect_text_operator_anomalies(txt: str, indicators: dict):
    ...
    try:
        ...
        tw_anomalies = 0
        if "Tw" in txt:
            tw_anomalies = len(re.findall(
                r"(?:^|[^0-9\.])(1[0-9]{2,}|-[1-9][0-9]+)\s+Tw\b", txt))
        tc_anomalies = 0
        if "Tc" in txt:
            tc_anomalies = len(re.findall(
                r"(?:^|[^0-9\.])(1[0-9]{2,}|-[1-9][0-9]+)\s+Tc\b", txt))
...
def detect_timestamp_mismatches(txt: str, doc, indicators: dict):
    ...
    try:
        ...
        xmp_create_match = re.search(
            r"<xmp:CreateDate>([^<]+)", txt) if "CreateDate" in txt else None
        xmp_mod_match = re.search(
            r"<xmp:ModifyDate>([^<]+)", txt) if "ModifyDate" in txt else None

        xmp_create = parse_pdf_date(
            xmp_create_match.group(1)) if xmp_create_match else None
        xmp_mod = parse_pdf_date(
            xmp_mod_match.group(1)) if xmp_mod_match else None

        mismatches = []
        if info_create and xmp_create and abs((info_create - xmp_create).total_seconds()) > 60:
            mismatches.append(
                f"Creation: Info({info_create.strftime('%Y-%m-%d')}) != XMP({xmp_create.strftime('%Y-%m-%d')})")

        if info_mod and xmp_mod and abs((info_mod - xmp_mod).total_seconds()) > 60:
            mismatches.append(
                f"Modify: Info({info_mod.strftime('%Y-%m-%d')}) != XMP({xmp_mod.strftime('%Y-%m-%d')})")
    ...
    try:
        ...
        if dimensions[dominant_dim] / len(doc) >= 0.75:
            for i in range(len(doc)):
                ...
                w_h = round(r.width, 1), round(r.height, 1)
                if w_h != dominant_dim:
                    anomalous_pages.append(
                        {'page': i+1, 'type': f'Dimensions {w_h}'})
        ...
        if rotations[dominant_rot] / len(doc) >= 0.75:
            for i in range(len(doc)):
                ...
                if rot != dominant_rot:
                    anomalous_pages.append(
                        {'page': i+1, 'type': f'Rotation {rot}° (expected {dominant_rot}°)'})
...
def detect_color_space_anomalies(doc, indicators: dict):
    ...
    if not doc:
        return
    try:
        color_spaces = {'DeviceRGB': 0, 'DeviceCMYK': 0,
                        'DeviceGray': 0, 'Indexed': 0, 'Other': 0}
        anomalies = []
            ...
            if color_spaces[dominant] / total > 0.8:
                for cs, count in color_spaces.items():
                    if cs != dominant and count > 0 and count <= total * 0.15:
                        anomalies.append(
                            f"{count} {cs} image(s) vs dominant {dominant}")
...
def detect_stacked_filters(doc, txt: str, indicators: dict):
    ...
    try:
        ...
        if re.search(r'/Filter\s*\[', txt):
            ...
            xref_count = doc.xref_length() if callable(
                doc.xref_length) else doc.xref_length
...
def detect_xmp_history_gaps(txt: str, indicators: dict):
    ...
    try:
        ...
        if "stEvt:when" in txt:
            items = re.findall(
                r"<rdf:li[^>]*stEvt:instanceID=\"([^\"]+)\"[^>]*stEvt:when=\"([^\"]+)\"", txt)
            if not items:
                # Try alternate XML format
                items = re.findall(
                    r"<stEvt:instanceID>([^<]+)</stEvt:instanceID>.*?<stEvt:when>([^<]+)</stEvt:when>", txt, re.S)
        ...
        if len(items) > 1:
            ...
            for i in range(len(items) - 1):
                try:
                    ...
                    # Convert to datetime
                    # ⚡ Bolt Optimization: Replace strptime with fromisoformat for speed
                    ...
                    if diff < 0:
                        gaps.append(
                            f"Reverse sequence: {items[i+1][1]} occurs before {items[i][1]}")
                    elif diff > 31536000 * 2:  # 2 years
                        gaps.append(
                            f"Large history gap (>2 years) between revisions")
                except Exception:
                    continue
...
def detect_structural_scrubbing(pdf_bytes: bytes, indicators: dict):
    ...
    try:
        ...
        if null_runs > 0:
            findings.append(
                f"Found {null_runs} block(s) of 200+ null bytes (potential scrubbing)")
        ...
        if space_runs > 0:
            findings.append(
                f"Found {space_runs} block(s) of 1000+ spaces (potential manual white-out)")
...
def detect_pdfa_violations(doc, txt: str, indicators: dict):
    ...
    try:
        ...
        if 'NonEmbeddedFont' in indicators:
            violations.append(
                "Document claims PDF/A but has non-embedded fonts")
